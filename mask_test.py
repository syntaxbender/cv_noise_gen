...
def apply_perlin_to_alpha(image, scale=50, min_alpha=0.6, max_alpha=1.0):
    ...
    img_r_channel = image[:, :, 0]  # 0-1 aralığında normalize et
    img_g_channel = image[:, :, 1]  # 0-1 aralığında normalize et
    img_b_channel = image[:, :, 2]  # 0-1 aralığında normalize et
    ...
    img_r_channel[mask] = 0.5
    img_g_channel[mask] = 0.5
    img_b_channel[mask] = 0.5
    # Güncellenmiş alpha kanalını uygula
    # image[:, :, 3] = img_alpha_channel
    image[:, :, 2] = img_b_channel
    image[:, :, 1] = img_g_channel
    image[:, :, 0] = img_r_channel
    return image
